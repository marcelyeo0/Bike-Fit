...
from draft.pose.mediapipe_estimator import MediaPipeEstimator
from draft.windtunnel.drag import DragResult, drag_score
from draft.windtunnel.flow import FlowField, solve_flow
...
class TunnelWorker(QThread):
    ...
    def __init__(self, frame_bgr: np.ndarray,
                 mask: np.ndarray | None = None, parent=None):
        super().__init__(parent)
        self._frame = frame_bgr
        self._mask = mask  # silhouette déjà connue (mode démo) → pas de pose

    def run(self) -> None:
        if self._mask is not None:
            # Mode démo : la silhouette est fournie, on passe directement
            # au calcul du flux (pas de détection, pas de nettoyage).
            flow = solve_flow(self._mask)
            self.result_ready.emit(TunnelResult(
                base_frame=self._frame, mask=self._mask, flow=flow,
                drag=drag_score(self._mask)))
            return
