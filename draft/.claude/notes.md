...
## Étape 6 — Option démo de la soufflerie (2026-07-11)

### Ce que je fais
Bouton « Démo — cycliste générique » sur la page soufflerie (demande de
Marcel) : visualiser le flux sans importer de vidéo, pour la présentation
client. Sélectionner la démo génère la scène et lance le calcul tout seul.

### Décisions
- **Pseudo-3D, pas vrai 3D** : la demande parlait d'un « cycliste en 3D »,
  mais la simulation est 2D par cadrage validé. Un vrai rendu 3D exigerait
  un maillage + moteur de rendu pour zéro gain physique. Compromis :
  silhouette procédurale (capsules entre articulations, posture aéro) avec
  ombrage volumique par distance transform (centre clair, bords sombres) —
  l'effet « rendu 3D stylisé » de la maquette, pour ~40 lignes.
- La démo est un **3e slot** à côté de A/B : elle réutilise tel quel le
  worker (qui saute MediaPipe quand la silhouette est fournie), le rendu
  (nouveaux paramètres dim/tint_person : la scène étant déjà stylisée, on
  ne l'assombrit pas) et le panneau de score. Le vélo est dessiné en décor
  mais exclu de la silhouette aérodynamique (seul le corps compte, cohérent
  avec le score des positions A/B).
- Silhouette déterministe (pas d'aléa) : même scène, même score (287) à
  chaque démo — important pour un discours commercial répétable.
